					...
					switch(clMessage.getType()) {
						case REGISTER:{ 
							...
							if(!taken){
								...
								sMessage = new ServerMessage(ServerMessage.ROUTE.NEW_USER);
								sMessage.setUser(name);
								for(Connection connect : all_connections){
									connect.out.writeObject(sMessage);
								}
							}
							break;
						}
						...
						case SHARE:{
							if(verified){
								for (String destination :  clMessage.getDestinationList() ) {
									ArrayList<URL> links ;
									for (URL link : (links=clMessage.getList()) ){
										SQLManager.shareLink(name,destination,link);
									}
									
									ServerMessage sMessage = new ServerMessage(ServerMessage.ROUTE.SHARED_LINKS);
									sMessage.setList(links);
									
									
									for(Connection connect : all_connections){
										if (connect.name.equals(destination)){
											connect.out.writeObject(sMessage);
										}
									}
								}
						...
						case SAVE:{
							if(verified){
								for (URL link : clMessage.getList() ){
									SQLManager.sharedLink(name,link,true);
								}
							}
							break;
						}
